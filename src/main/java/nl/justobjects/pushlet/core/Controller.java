...
import com.chart.cn.core.engine.UserSessionMap;
...
public class Controller implements Protocol, ConfigDefs {
	...
	public void doCommand(Command aCommand) {
		try {
			...
			if (eventType.equals(Protocol.E_REFRESH)) {
				...
				doListen(aCommand);
			} else if(eventType.equals("showOnlineUser")){
				//测试添加的所有用户并进行输出
				doShowAllUser(aCommand);
			} else if(eventType.equals("findOnlineByUsername")){
				//查看该用户是否上线如是，则返回用户ID
				doFindOnlineByUsername(aCommand);
			}
	...
	protected void doShowAllUser(Command aCommand) {

		UserSessionMap.getInstance().showALlOnlineUserName();
		aCommand.setResponseEvent(new Event(E_HEARTBEAT_ACK));
	}
	
	protected void doFindOnlineByUsername(Command aCommand) throws IOException {

		String findName = aCommand.reqEvent.getField("findName");
		System.out.println(findName);
		String userId = UserSessionMap.getInstance().getSessionId(findName);
		BufferedReader reader = aCommand.httpReq.getReader(); 
		String line = null;    
        while((line = reader.readLine()) != null) {    
            System.out.println(line);    
        }
		aCommand.httpRsp.setContentType("tex/html");
		PrintWriter out = aCommand.httpRsp.getWriter(); 
		out.println("{findnameid:"+findName+"}");    
		out.flush();    
		out.close(); 
		aCommand.setResponseEvent(new Event(E_HEARTBEAT_ACK));
	}
	...
	protected void doPublish(Command aCommand) {
		...
		try {
			...
			if (subject == null) {
				...
				if("/user/login".equals(subject)){
					//更新在线用户列表
					UserSessionMap.getInstance().addNewUerSession(aCommand.reqEvent.getField("name"), session.getId());
				}
