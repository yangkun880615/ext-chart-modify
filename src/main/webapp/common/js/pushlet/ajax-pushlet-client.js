...
// Do XML HTTP request
	_doRequest: function(anEvent, aQuery) {
		...
		//网上找的 网址 http://blog.csdn.net/xiaxiaorui2003/article/details/16120013
	    if (anEvent == 'join' || anEvent == 'join-listen') {  
	    	url = url +"&userSession="+PL.userSession;   
	    }  
		...
		if (PL.sessionId != null) {
			...
			console.log("PL.sessionId="+PL.sessionId);
			if (anEvent == 'p_leave') {
				PL.sessionId = null;
			}
		}
